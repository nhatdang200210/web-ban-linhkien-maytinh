...
if (isset($_POST['repcoment'])) {
    ...
    if (isset($_POST['reply']) && isset($_POST['id_danhgia']) && !empty($_POST['reply'])) {
        ...
        echo "<script>
            ...
            window.location ='../../index.php?action=danhgia&query=darep';
        </script>";
    } else {
        echo "<script>
            alert('Bạn hãy nhập nội dung để phản hồi.');
            window.location ='../../index.php?action=quanlydanhgia&query=danhgiachitiet';
        </script>";
    }
    // echo "<script>
    //         alert('Bạn đã trả lời bình luận thành công.');
    //         window.location ='http://localhost/Web-NLN/admin/index.php?action=quanlydanhgia&query=danhgiachitiet_rep&id=$id_sanpham';
    //     </script>"; 
}
else{
    $idrep=$_GET['idrep'];
